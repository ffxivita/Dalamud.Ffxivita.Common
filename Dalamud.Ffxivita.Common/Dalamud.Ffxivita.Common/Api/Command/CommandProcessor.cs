...
namespace Dalamud.Ffxivita.Common.Api.Command
{
    internal sealed partial class CommandProcessor : ICommandProcessor
    {
        ...
        private readonly List<FfxivitaCommand> commands = new();
        ...
        public IReadOnlyList<FfxivitaCommand> Commands
        {
            get
            {
                lock (commandsLock)
        ...
        public void RegisterCommandsByAttribute(ICommandProvider? instance)
        {
            ...
            // public/internal/protected/private/static Trova i metodi e ordinali
                ...
                .Concat(instance.GetType().GetMethods(flags | BindingFlags.DeclaredOnly).OrderBy(x => x.MetadataToken)))
            {
                foreach (var attribute in method.GetCustomAttributes<CommandAttribute>())
                {
                    try
                    {
                        var command = new FfxivitaCommand(method,
                            instance,
                            attribute,
                            Prefix ?? $"/{pluginName}".Replace("FFXIVITA.", string.Empty).ToLower(),
                            pluginName);
        ...
        private void RegisterCommand(FfxivitaCommand command)
        {
            commands.Add(command);
            PluginLog.Information("comando: {Usage} è stato registrato. Regex = {Regex}, Priority = {Priority}",
                command.Usage,
                command.Regex,
                command.Priority);
            ...
            chatClient.Print(payloads =>
            {
                ...
                payloads.AddRange(new List<Payload>
                {
                    ...
                    new TextPayload("è stato aggiunto."),
                });
